...
def draw_menu_background():
    ...
    for arrow in menu_arrows:
        ...
        if arrow[2] - 30 > HEIGHT:
            ...
            arrow[1] = arrow[0] * lane_width + lane_width // 2
...
class Character:
    def __init__(self, image_path):
        ...
        # Posizione a destra della schermata
        self.base_x = WIDTH - 100  # 100 pixel dal bordo destro
        self.base_y = 520
        self.x = self.base_x
        self.y = self.base_y
    ...
    while running:
        ...
        if game_state in ["PLAYING", "PAUSED", "OPTIONS"]:
            ...
            if rating_timer > 0:
                ...
                rating_rect = animated_rating_surf.get_rect(center=(0 * lane_width + lane_width // 2, hit_line_y - 120))
            ...
            stats_labels = [("PERFECT:", counts["PERFECT"], GOLD), ("GOOD:", counts["GOOD"], GREEN), ("BAD:", counts["BAD"], ORANGE), ("TRASH:", counts["TRASH"], RED), ("MISS:", counts["MISS"], DARK_RED)]
            for i, (label, val, col) in enumerate(stats_labels):
                stat_surf = stats_font.render(f"{label} {val}", True, col)
                screen.blit(stat_surf, (WIDTH - stat_surf.get_width() - 20, HEIGHT - 130 + (i * 22)))
